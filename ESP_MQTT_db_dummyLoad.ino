...
// 01:00 - 12:00 = jalan datar  -> relay resistor OFF
// 12:00 - 23:00 = jalan tanjakan -> relay resistor ON, hanya saat MODE_LOAD
// 23:00 - 01:00 = istirahat/charge-only -> relay load OFF, relay resistor OFF
...
void updateRelayOutput() {
  // Relay resistor hanya boleh ON saat sistem benar-benar sedang LOAD
  // dan profil kalender berada pada mode tanjakan.
...
void setup() {
  ...
  Serial.println("JK BMS BLE + SAFETY RELAY CONTROL + ENCODER + MQTT + CALENDAR ROAD PROFILE");
